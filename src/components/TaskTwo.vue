  ...
  <div class="common-layout">
    <el-container class="box">
      <el-header style="margin-top: 20px;">
        <div class="title" style="font-size: 25px;">多级反馈队列轮转调度算法模拟</div>
      </el-header>
      <el-container style=" width: 960px; margin-top: 0px;">
        <!-- <el-aside width="200px" style="border: solid 1px red"></el-aside> -->
        <el-container>
          <el-main width="950px">
            <div class="choose" style="margin-left: -15px;">
              <el-form-item label="一级队列时间片" size="default" style="font-size: 20px;" label-width="130px">
                <el-input-number v-model="l1Time" />
              </el-form-item>
              <el-form-item label="二级队列时间片" size="default" label-width="130px">
                <el-input-number v-model="l2Time" />
              </el-form-item>
              <el-form-item label="三级队列时间片" size="default" label-width="130px">
                <el-input-number v-model="l3Time" />
              </el-form-item>
            </div>
            <div class="content">
              <el-table :data="tableData"
                style=" font-size: 16px;border: solid 1px black ;width:440px;min-height: 240px;">
    ...
    <el-dialog v-model="dialogResultVisible" title="模拟结果" style="width: 650px; ">
      ...
      <div style="margin-top: 50px;" v-for="(item, index) in waitList" :key="index">
        ...
        <el-table :data="item.q" style=" border: solid 1px blueviolet ;width:600px" max-height="900">
          ...
          <el-table-column prop="listName" label="所在队列" width="120" />
...
import PriorityQueueArray from "../utils/PriorityQueueArray"
  ...
  methods: {
    draw() {
      ...
      for (let i = 0; i < this.resultData.length; i++) {
        ...
        // aTime.push(this.resultData[i].aTime)
        // leaveTime.push(this.resultData[i].leaveTime)
      }
      const data = {}
      for (let i = 0; i < this.resultData.length; i++) {
      ...
      // for (let i in data) {
      //   console.log(i , this.resultData[data[i]].name)
      // }
      const result = []
    ...
    startSimulation() {
      var timeSlice = []
      timeSlice.push(this.l1Time);
      timeSlice.push(this.l2Time);
      timeSlice.push(this.l3Time);
      var arr2 = []
      for (var i = 0; i < this.tableData.length; i++) {
        var process = new Process(this.tableData[i].name, this.tableData[i].aTime, this.tableData[i].rTime, 1, timeSlice[0])
        // arr1.push(process)
        arr2.push(process)
      }
      // var pro=new Process()
      ...
      // var p1=new PriorityQueue(1,arr1) 
      var p2 = new PriorityQueue(2, arr2)
      // p.print()
      var t = 0;
      var p1 = new PriorityQueueArray()
      var s = 0;
      var infomation = []
      var pro
      var processList = []
      ...
      while (s < this.tableData.length) {
        ...
        if (diff > p1.peak().timeSlice) {
          if (p1.peak().timeSlice < p1.peak().rTime) {
            ...
            infomation.push({ t: "进程" + p1.peak().name + "执行完一个时间片，回到下一级队列中。" })
            pro = p1.pop()
            pro.state = "就绪"
            pro.Priority += 1
            if (pro.Priority > 3) {
            ...
            pro.timeSlice = timeSlice[pro.Priority - 1]
            p1.push(pro)
            p1.peak().state = "执行"
            infomation.push({ t: "进程" + p1.peak().name + "开始执行。" })
            p1.peak().runningTime.push(t)
            // waitPro=[]
            // waitPro=(processList.concat(p1.sort())).concat(p2.sort)
          ...
          if (p1.peak().timeSlice < p1.peak().rTime) {
            ...
            if (p2.getSize() != 0) {
              ...
              if (p1.peak() == pro) {
                ...
                infomation.push({ t: "进程" + p1.peak().name + "开始执行。" })
                waitList.push({ time: t, info: infomation[infomation.length - 2].t + infomation[infomation.length - 1].t + infomation[infomation.length - 3], q: (processList.concat(p1.sort())).concat(p2.sort()) })
              }
            }
            else {
              p1.peak().runningTime.push(t)
          ...
          if (diff > p1.peak().rTime) {
            ...
            waitList.push({ time: t, info: infomation[infomation.length - 1].t + infomation[infomation.length - 2].t, q: (processList.concat(p1.sort())).concat(p2.sort()) })
            s++
          }
          else {
            t += diff
